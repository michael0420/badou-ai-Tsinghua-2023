...
img_jiance = img_yizhi
lower_boundary = img_tidu.mean() * 0.5
high_boundary = lower_boundary * 3  # 这里我设置高阈值是低阈值的三倍
zhan = []
for i in range(1, img_jiance.shape[0] - 1):  # 外圈不考虑了
    for j in range(1, img_jiance.shape[1] - 1):
        if img_jiance[i, j] >= high_boundary:  # 取，一定是边的点
            img_jiance[i, j] = 255
            zhan.append([i, j])
        elif img_jiance[i, j] <= lower_boundary:  # 舍
            img_jiance[i, j] = 0

while not len(zhan) == 0:
    temp_1, temp_2 = zhan.pop()  # 出栈
    a = img_jiance[temp_1 - 1:temp_1 + 2, temp_2 - 1:temp_2 + 2]
    if (a[0, 0] < high_boundary) and (a[0, 0] > lower_boundary):
        img_jiance[temp_1 - 1, temp_2 - 1] = 255  # 这个像素点标记为边缘
        zhan.append([temp_1 - 1, temp_2 - 1])  # 进栈
    if (a[0, 1] < high_boundary) and (a[0, 1] > lower_boundary):
        img_jiance[temp_1 - 1, temp_2] = 255
        zhan.append([temp_1 - 1, temp_2])
    if (a[0, 2] < high_boundary) and (a[0, 2] > lower_boundary):
        img_jiance[temp_1 - 1, temp_2 + 1] = 255
        zhan.append([temp_1 - 1, temp_2 + 1])
    if (a[1, 0] < high_boundary) and (a[1, 0] > lower_boundary):
        img_jiance[temp_1, temp_2 - 1] = 255
...
for i in range(img_jiance.shape[0]):
    for j in range(img_jiance.shape[1]):
        if img_jiance[i, j] != 0 and img_jiance[i, j] != 255:
            img_jiance[i, j] = 0

# 绘图
plt.figure(4)
plt.imshow(img_jiance.astype(np.uint8), cmap='gray')
